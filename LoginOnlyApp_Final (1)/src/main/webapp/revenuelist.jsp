...
<%
    if (searchQuery != null && !searchQuery.trim().isEmpty()) {
        if ("flight".equals(searchBy)) {
            ...
            out.println("<h3>Total Revenue from Flight ID '" + searchQuery + "': $" + String.format("%.2f", totalRevenue) + "</h3>");

            ps = con.prepareStatement("SELECT u.id, u.username, u.first_name, u.last_name, f.price " +
...
<%
            ...
            ps = con.prepareStatement("SELECT SUM(f.price) AS total_revenue FROM bookings b " +
            ...
            out.println("<h3>Total Revenue from Airline '" + searchQuery + "': $" + String.format("%.2f", totalRevenue) + "</h3>");

            ps = con.prepareStatement("SELECT f.flight_id, SUM(f.price) AS revenue FROM bookings b " +
...
<%
            ...
            out.println("<h3>Total Revenue from Customer '" + searchQuery + "': $" + String.format("%.2f", totalRevenue) + "</h3>");

            ps = con.prepareStatement("SELECT u.id, u.username, u.first_name, u.last_name, b.booking_id, f.airline, f.flight_id, f.price, b.ticket_class " +
                "FROM bookings b JOIN users u ON b.user_id = u.id " +
                "JOIN flights f ON b.flight_id = f.flight_id " +
                "WHERE u.username LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ?");
            ps.setString(1, like);
...
%>
            <table border="1">
                <tr><th>User ID</th><th>Username</th><th>First</th><th>Last</th><th>Booking ID</th><th>Class</th><th>Airline</th><th>Flight ID</th><th>Revenue</th></tr>
<%
            while (rs.next()) {
%>
                <tr>
