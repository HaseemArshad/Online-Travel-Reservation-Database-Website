    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("edit_view".equals(action)) {
                String[] keys = {
                    "flight_id", "airline", "from_airport", "to_airport", "departure_date",
                    "departure_time", "arrival_time", "price", "stops", "capacity"
                };
                for (String key : keys) {
                ...
                String sql = "INSERT INTO flights (flight_id, airline, from_airport, to_airport, departure_date, departure_time, arrival_time, price, stops, capacity) " +
                             "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                String sql = "UPDATE flights SET airline=?, from_airport=?, to_airport=?, departure_date=?, departure_time=?, arrival_time=?, price=?, stops=?, capacity=? " +
                             "WHERE flight_id=?";
                ...
                String depTimeRaw = request.getParameter("departure_time");
                if (!depTimeRaw.matches("^\\d{2}:\\d{2}:\\d{2}$")) depTimeRaw += ":00";
                ps.setTime(5, java.sql.Time.valueOf(depTimeRaw));

                String arrTimeRaw = request.getParameter("arrival_time");
                if (!arrTimeRaw.matches("^\\d{2}:\\d{2}:\\d{2}$")) arrTimeRaw += ":00";
                ps.setTime(6, java.sql.Time.valueOf(arrTimeRaw));

                ps.setBigDecimal(7, new BigDecimal(request.getParameter("price")));
                ps.setInt(8, Integer.parseInt(request.getParameter("stops")));
                ps.setInt(9, Integer.parseInt(request.getParameter("capacity")));
        ...
        try (PreparedStatement fetch = con.prepareStatement("SELECT * FROM flights");
            ...
            while (rs.next()) {
                flightList.add(new String[] {
                        rs.getString("flight_id"), rs.getString("airline"), rs.getString("from_airport"),
                        rs.getString("to_airport"), rs.getString("departure_date"), rs.getString("departure_time"),
                        rs.getString("arrival_time"), rs.getString("price"), rs.getString("stops"),
